...
namespace APJ.NET.Services
{
    public class UsersService
    {
        ...
        public void AddUsers(Users model)
        {
        ...
        public IList<Users> GetAllUsers()
        {
            ...
            return dao.FindAll<Users>();
        }

        public PaginatedList SearchUsers(Dictionary<string, object> args, int page, int limit)
        {
            ...
            {
                connection.Open();
                data.Data = SearchQuery.findWherePaginated<Users>(connection, page, limit, ApjDaoUtil.MakeSearchCriteria(typeof(Users), args).ToArray());
                data.TotalCount = AggregateQuery.CountWhere<Users>(connection);
                data.Page = page;
                data.Limit = limit;
                return data;
            }
        ...
        public Users ChangeUsersState(string id, int state)
        {
            DbConnection connection = ConnectionFactory.GetConnection();
            try
            {
        ...
        public Users DeleteUsers(string id, bool permanently)
        {
            if(!permanently)
            {
                return ChangeUsersState(id, State.DELETED);
            }
            else
            {
                DbConnection connection = ConnectionFactory.GetConnection(); ;
