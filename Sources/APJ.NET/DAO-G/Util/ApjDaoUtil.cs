...
namespace Apj.Net.Dao.Util
{
    public class ApjDaoUtil
    {
        ...
        internal static IList<M> Load<M>(DbDataReader rdr) where M : BaseModel
        {
            ...
            while(rdr.Read())
            {
                ...
                for(int i=0; i<rdr.FieldCount; i++)
                {
                    ...
                    Console.WriteLine("Load "+columnName);
                    PropertyInfo p = GetPropertyByName(c, columnName);
                    if(p != null)
                    {
                        var value = rdr[columnName];
                        value = value = value == DBNull.Value ? null : value;
        ...
        {
            if (model.GetIndicePK() == null) throw new DAOException("Indice PK non spécifiée");
