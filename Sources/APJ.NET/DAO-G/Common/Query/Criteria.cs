...
namespace Apj.Net.Dao.Common.Query
{
    public class Criteria
    {
        ...
        {
            return new Criteria(column, " is null ", null);
        }

        public static Criteria IsNotNull(string column)
        {
            return new Criteria(column, " is not null ", null);
        }
        ...
        private static string GetQueryParameterKey(string str)
        {
            ...
            switch(Config.DB_NAME)
            {
                case Constante.SQLSERVER_DB:
                    s = RemoveOrAnd(s);
                    return Constante.SQLSERVER_QUERY_PARAMETER_ID + s.TrimStart();
                case Constante.ORACLE_DB:
                    s = RemoveOrAnd(s);
                    return Constante.ORACLE_QUERY_PARAMETER_ID + s.TrimStart();
                default: return "?";
            }
        } 
